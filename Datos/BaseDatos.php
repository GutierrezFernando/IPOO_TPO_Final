...
class BaseDatos {
    ...
    public function Registro() {
        ...
        if ($this->RESULT){
            unset($this->ERROR);
            if($temp = mysqli_fetch_assoc($this->RESULT)){  // mysqli_fetch_assoc: retorna un array asociativo correspondiente a la fila obtenida o null si no hubiera más filas.
                $resp = $temp;
            }else{
                mysqli_free_result($this->RESULT);  // La función mysqli_free_result() libera la memoria asociada al resultado representado por el parámetro resultado , la cuál fue tomada por mysqli_query(), mysqli_store_result() o mysqli_use_result().
                                                    // Note: Usted debe siempre liberar el resultado con mysqli_free_result(), cuando su objeto resultado no se necesite más.
            }
        }else{
            $this->ERROR = mysqli_errno($this->CONEXION) . ": " . mysqli_error($this->CONEXION);
        }
